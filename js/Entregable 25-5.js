//DESAFIO CON CONDICIONALES, FUNCIONES, ARREGLOS
/**
...
class FigurasDeYeso {
  //este constructor me sirve para instanciar un objeto
  constructor(figura) {
    this.id = figura.id;
...
function menuFigurasVenta() {
  // esta función es para que el usuario seleccione en menu los productos en stock y se agreguen en carrito
  let listaFigurasString = "";
  for (const fig of figuras) {
    //recorro el arreglo de objetos para arrojar una lista de opciones en el menú de ventas
    listaFigurasString += `
...
${listaFigurasString} \n`);
    ...
    let figuraSeleccionada = carritoCompras.find(
      (elem) => elem.id == idFigurasVenta
    ); //con el find me arroja el primer elemento que en este caso es un objeto del arreglo, pero necesito el index de esta busqueda que hice con find, para solo ver el id y compararla con el prompt

    if (figuraSeleccionada) {
      let indexCarrito = carritoCompras.findIndex(
        (elem) => elem.id === figuraSeleccionada.id
      );
...
Unidades: ${carritoCompras[indexCarrito].cantidad} Precio Und: ${carritoCompras[indexCarrito].precio}`);
  ...
  carritoCompras.forEach((carrito) => {
    console.table(
      `Agregaste ${carrito.modelo} a tu carrito de compras y el total son: ${carrito.precioTotal}`
    );
  });
  let totalAPago = 0;
  carritoCompras.forEach((carrito) => (totalAPago += carrito.precioTotal));
  console.table(`El total de tu carrito son: ${totalAPago}`);
}
...
console.log(
  "LISTAR LOS NOMBRES DE LOS PRODUCTOS CON DCTO CON PAGO CONTADO + IMPTO"
);
const precioEfectivo = figuras.map((elem) => {
  return {
    ...
    precioMasImpuesto: elem.precio * 0.9 * 1.19,
  };
});
...
const precioCreditoMasImpuesto = figuras.map((elem) => {
  return {
    ...
    precioMasImpuesto: elem.precio * 1.02 * 1.19,
  };
});
